...
/// <summary>Represents a SA-MP player.</summary>
public partial class BasePlayer : IdentifiedPool<BasePlayer>, IWorldObject
{
    ...
    /// <summary>Initializes a new instance of the <see cref="BasePlayer" /> class.</summary>
    public BasePlayer()
    {
    ...
    /// <summary>Gets or sets the default color of a client message.</summary>
    public static Color DefaultClientMessageColor { get; set; }

    /// <summary>Gets a collections of Player Variables of this Player.</summary>
    public PVarCollection PVars { get; }

    /// <summary>Gets a set of KeyHandlers for different key states.</summary>
    public KeyChangeHandlerSet Key { get; }

    /// <summary>Gets the size of the players pool.</summary>
    ...
    /// <summary>Gets or sets the position of the camera of this Players.</summary>
    ...
    public virtual GlobalObject CameraTargetGlobalObject
    {
        get
        {
            ...
            return id == GlobalObject.InvalidId
                ? null
                : GlobalObject.Find(id);
        }
    }
    ...
    public virtual PlayerObject CameraTargetPlayerObject
    {
        get
        {
            ...
            var id = PlayerInternal.Instance.GetPlayerCameraTargetObject(Id);
            return id == PlayerObject.InvalidId
                ? null
                : PlayerObject.Find(this, id);
        }
    }
    ...
    /// <summary>Gets the Vehicle that this Player is surfing.</summary>
    public virtual BaseVehicle SurfingVehicle
    {
        get
        {
            var vehicleid = PlayerInternal.Instance.GetPlayerSurfingVehicleID(Id);
            return vehicleid == BaseVehicle.InvalidId
                ? null
                : BaseVehicle.Find(vehicleid);
        }
    }

    /// <summary>Gets the <see cref="GlobalObject" /> that this Player is surfing.</summary>
    public virtual GlobalObject SurfingGlobalObject
    {
        get
        {
            var objectid = PlayerInternal.Instance.GetPlayerSurfingObjectID(Id);
            return objectid == GlobalObject.InvalidId
                ? null
                : GlobalObject.Find(objectid);
        }
    }

    /// <summary>Gets the <see cref="PlayerObject" /> that this Player is surfing.</summary>
    public virtual PlayerObject SurfingPlayerObject
    {
        get
        {
            var objectid = PlayerInternal.Instance.GetPlayerSurfingObjectID(Id);
            return objectid == PlayerObject.InvalidId
                ? null
                : PlayerObject.Find(this, objectid);
        }
    }
    ...
    /// <summary>Gets whether this Player is connected to the server.</summary>
    ...
    public virtual ConnectionStatus ConnectionStatus =>
        (ConnectionStatus)PlayerInternal.Instance.GetConnectionStatus(Id);


    /// <summary>Occurs when the <see cref="OnConnected" /> is being called. This callback is called when a player connects to the server.</summary>
    public event EventHandler<EventArgs> Connected;

    /// <summary>Occurs when the <see cref="OnDisconnected" /> is being called. This callback is called when a player disconnects from the server.</summary>
    public event EventHandler<DisconnectEventArgs> Disconnected;

    /// <summary>Occurs when the <see cref="OnCleanup" /> is being called. This callback is called after a player disconnects from the server.</summary>
    ...
    /// <summary>
    /// Occurs when the <see cref="OnCommandText" /> is being called. This callback is called when a player enters a command into the client chat window, e.g.
    /// /help.
    /// </summary>
    public event EventHandler<CommandTextEventArgs> CommandText;

    /// <summary>
    /// Occurs when the <see cref="OnRequestClass" /> is being called. Called when a player changes class at class selection (and when class selection first
    /// appears).
    /// </summary>
    public event EventHandler<RequestClassEventArgs> RequestClass;

    /// <summary>
    /// Occurs when the <see cref="OnEnterVehicle" /> is being called. This callback is called when a player starts to enter a vehicle, meaning the player is
    /// not in vehicle yet at the time this callback is called.
    /// </summary>
    public event EventHandler<EnterVehicleEventArgs> EnterVehicle;

    /// <summary>Occurs when the <see cref="OnExitVehicle" /> is being called. This callback is called when a player exits a vehicle.</summary>
    /// <remarks>Not called if the player falls off a bike or is removed from a vehicle by other means such as setting <see cref="Position" />.</remarks>
    public event EventHandler<PlayerVehicleEventArgs> ExitVehicle;

    /// <summary>Occurs when the <see cref="OnStateChanged" /> is being called. This callback is called when a player exits a vehicle.</summary>
    /// <remarks>Not called if the player falls off a bike or is removed from a vehicle by other means such as setting <see cref="Position" />.</remarks>
    public event EventHandler<StateEventArgs> StateChanged;

    /// <summary>Occurs when the <see cref="OnEnterCheckpoint" /> is being called. This callback is called when a player enters the checkpoint set for that player.</summary>
    public event EventHandler<EventArgs> EnterCheckpoint;

    /// <summary>Occurs when the <see cref="OnLeaveCheckpoint" /> is being called. This callback is called when a player leaves the checkpoint set for that player.</summary>
    public event EventHandler<EventArgs> LeaveCheckpoint;

    /// <summary>Occurs when the <see cref="OnEnterRaceCheckpoint" /> is being called. This callback is called when a player enters a race checkpoint.</summary>
    public event EventHandler<EventArgs> EnterRaceCheckpoint;

    /// <summary>Occurs when the <see cref="OnLeaveRaceCheckpoint" /> is being called. This callback is called when a player leaves the race checkpoint.</summary>
    public event EventHandler<EventArgs> LeaveRaceCheckpoint;

    /// <summary>Occurs when the <see cref="OnRequestSpawn" /> is being called. Called when a player attempts to spawn via class selection.</summary>
    public event EventHandler<RequestSpawnEventArgs> RequestSpawn;

    /// <summary>Occurs when the <see cref="OnEnterExitModShop" /> is being called. This callback is called when a player enters or exits a mod shop.</summary>
    public event EventHandler<EnterModShopEventArgs> EnterExitModShop;

    /// <summary>Occurs when the <see cref="OnSelectedMenuRow" /> is being called. This callback is called when a player selects an item from a menu.</summary>
    public event EventHandler<MenuRowEventArgs> SelectedMenuRow;

    /// <summary>Occurs when the <see cref="OnExitedMenu" /> is being called. Called when a player exits a menu.</summary>
    public event EventHandler<EventArgs> ExitedMenu;

    /// <summary>Occurs when the <see cref="OnInteriorChanged" /> is being called. Called when a player changes interior.</summary>
    /// <remarks>This is also called when <see cref="Interior" /> is set.</remarks>
    public event EventHandler<InteriorChangedEventArgs> InteriorChanged;

    /// <summary>
    /// Occurs when the <see cref="OnKeyStateChanged" /> is being called. This callback is called when the state of any supported key is changed
    /// (pressed/released). Directional keys do not trigger this callback.
    /// </summary>
    public event EventHandler<KeyStateChangedEventArgs> KeyStateChanged;

    /// <summary>Occurs when the <see cref="OnUpdate" /> is being called. This callback is called every time a client/player updates the server with their status.</summary>
    /// <remarks>This callback is called very frequently per second per player, only use it when you know what it's meant for.</remarks>
    public event EventHandler<PlayerUpdateEventArgs> Update;

    /// <summary>Occurs when the <see cref="OnStreamIn" /> is being called. This callback is called when a player is streamed by some other player's client.</summary>
    public event EventHandler<PlayerEventArgs> StreamIn;

    /// <summary>Occurs when the <see cref="OnStreamOut" /> is being called. This callback is called when a player is streamed out from some other player's client.</summary>
    public event EventHandler<PlayerEventArgs> StreamOut;

    /// <summary>
    /// Occurs when the <see cref="OnDialogResponse" /> is being called. This callback is called when a player responds to a dialog shown using
    /// <see cref="Dialog" /> by either clicking a button, pressing ENTER/ESC or double-clicking a list item (if using a list style dialog).
    /// </summary>
    public event EventHandler<DialogResponseEventArgs> DialogResponse;

    /// <summary>Occurs when the <see cref="OnTakeDamage" /> is being called. This callback is called when a player takes damage.</summary>
    public event EventHandler<DamageEventArgs> TakeDamage;

    /// <summary>Occurs when the <see cref="OnGiveDamage" /> is being called. This callback is called when a player gives damage to another player.</summary>
    /// <remarks>
    /// One thing you can do with GiveDamage is detect when other players report that they have damaged a certain player, and that player hasn't taken any
    /// health loss. You can flag those players as suspicious. You can also set all players to the same team (so they don't take damage from other players) and process
    /// all health loss from other players manually. You might have a server where players get a wanted level if they attack Cop players (or some specific class). In
    /// that case you might trust GiveDamage over TakeDamage. There should be a lot you can do with it. You just have to keep in mind the levels of trust between
    /// clients. In most cases it's better to trust the client who is being damaged to report their health/armour (TakeDamage). SA-MP normally does this. GiveDamage
    /// provides some extra information which may be useful when you require a different level of trust.
    /// </remarks>
    public event EventHandler<DamageEventArgs> GiveDamage;

    /// <summary>
    /// Occurs when the <see cref="OnClickMap" /> is being called. This callback is called when a player places a target/waypoint on the pause menu map (by
    /// right-clicking).
    /// </summary>
    /// <remarks>
    /// The Z value provided is only an estimate; you may find it useful to use a plugin like the MapAndreas plugin to get a more accurate Z coordinate (or
    /// for teleportation; use <see cref="SetPositionFindZ" />).
    /// </remarks>
    public event EventHandler<PositionEventArgs> ClickMap;

    /// <summary>
    /// Occurs when the <see cref="OnClickTextDraw" /> is being called. This callback is called when a player clicks on a textdraw. It is not called when
    /// player cancels the select mode (ESC)
    /// </summary>
    /// <remarks>
    /// The click-able area is defined by <see cref="TextDraw.Width" /> and <see cref="TextDraw.Width" />. The x and y parameters passed to that function must
    /// not be zero or negative.
    /// </remarks>
    public event EventHandler<ClickTextDrawEventArgs> ClickTextDraw;

    /// <summary>
    /// Occurs when the <see cref="OnClickPlayerTextDraw" /> is being called. This callback is called when a player clicks on a player-textdraw. It is not
    /// called when player cancels the select mode (ESC).
    /// </summary>
    public event EventHandler<ClickPlayerTextDrawEventArgs> ClickPlayerTextDraw;

    /// <summary>Occurs when the <see cref="OnCancelClickTextDraw" /> is being called. This callback is called when a player cancels the textdraw select mode(ESC).</summary>
    public event EventHandler<PlayerEventArgs> CancelClickTextDraw;

    /// <summary>Occurs when the <see cref="OnClickPlayer" /> is being called. Called when a player double-clicks on a player on the scoreboard.</summary>
    /// <remarks>
    /// There is currently only one 'source' (<see cref="PlayerClickSource.Scoreboard" />). The existence of this argument suggests that more sources may be
    /// supported in the future.
    /// </remarks>
    public event EventHandler<ClickPlayerEventArgs> ClickPlayer;

    /// <summary>Occurs when the <see cref="OnEditGlobalObject" /> is being called. This callback is called when a player ends object edition mode.</summary>
    public event EventHandler<EditGlobalObjectEventArgs> EditGlobalObject;

    /// <summary>Occurs when the <see cref="OnEditPlayerObject" /> is being called. This callback is called when a player ends object edition mode.</summary>
    public event EventHandler<EditPlayerObjectEventArgs> EditPlayerObject;

    /// <summary>Occurs when the <see cref="OnEditAttachedObject" /> is being called. This callback is called when a player ends attached object edition mode.</summary>
    /// <remarks>
    /// Editions should be discarded if response was '0' (cancelled). This must be done by storing the offsets etc. in an array BEFORE using
    /// EditAttachedObject.
    /// </remarks>
    public event EventHandler<EditAttachedObjectEventArgs> EditAttachedObject;

    /// <summary>
    /// Occurs when the <see cref="OnSelectGlobalObject" /> is being called. This callback is called when a player selects an object after
    /// <see cref="GlobalObject.Select" /> has been used.
    /// </summary>
    public event EventHandler<SelectGlobalObjectEventArgs> SelectGlobalObject;

    /// <summary>
    /// Occurs when the <see cref="OnSelectPlayerObject" /> is being called. This callback is called when a player selects an object after
    /// <see cref="PlayerObject.Select" /> has been used.
    /// </summary>
    public event EventHandler<SelectPlayerObjectEventArgs> SelectPlayerObject;

    /// <summary>Occurs when the <see cref="OnWeaponShot" /> is being called. This callback is called when a player fires a shot from a weapon.</summary>
    /// <remarks>
    /// <see cref="BulletHitType.None" />: the fX, fY and fZ parameters are normal coordinates; Others: the fX, fY and fZ are offsets from the center of hit
    /// object.
    /// </remarks>
    public event EventHandler<WeaponShotEventArgs> WeaponShot;

    /// <summary>Occurs when the <see cref="OnPickUpPickup" /> is being called. This callback is called when a player picks up a pickup.</summary>
        ...
        Weapon weapon2 = Weapon.None, int weapon2Ammo = 0, Weapon weapon3 = Weapon.None, int weapon3Ammo = 0)
    {
        ...
        PlayerInternal.Instance.SetSpawnInfo(Id, team, skin, position.X, position.Y, position.Z, rotation, (int)weapon1, weapon1Ammo, (int)weapon2, weapon2Ammo,
    ...
    /// <summary>This sets this <see cref="BasePlayer" />'s position then adjusts the Player's z-coordinate to the nearest solid ground under the position.</summary>
    ...
    /// <summary>Check if this <see cref="BasePlayer" /> is in range of a point.</summary>
    ...
    /// <summary>Calculate the distance between this <see cref="BasePlayer" /> and a map coordinate.</summary>
    ...
    /// <summary>Checks if a <see cref="BasePlayer" /> is streamed in this <see cref="BasePlayer" />'s client.</summary>
    ...
    /// <summary>Set the ammo of this <see cref="BasePlayer" />'s weapon.</summary>
    ...
    /// <summary>Give this <see cref="BasePlayer" /> a <see cref="Weapon" /> with a specified amount of ammo.</summary>
    ...
    /// <summary>Removes all weapons from this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Sets the armed weapon of this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Get the <see cref="Weapon" /> and ammo in this <see cref="BasePlayer" />'s weapon slot.</summary>
    ...
    /// <summary>Give money to this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Reset this <see cref="BasePlayer" />'s money to $0.</summary>
    ...
    /// <summary>Check which keys this <see cref="BasePlayer" /> is pressing.</summary>
    /// <remarks>
    ...
    /// <summary>Sets the clock of this <see cref="BasePlayer" /> to a specific value. This also changes the daytime. (night/day etc.)</summary>
    ...
    /// <summary>
    /// Get this <see cref="BasePlayer" />'s current game time. Set by <see cref="Server.SetWorldTime" />, <see cref="Server.SetWorldTime" />, or by
    ...
    /// <summary>Show/Hide the in-game clock (top right corner) for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>
    /// Set this <see cref="BasePlayer" />'s weather. If <see cref="ToggleClock" /> has been used to enable the clock, weather changes will interpolate
    /// (gradually change), otherwise will change instantly.
    /// </summary>
    ...
    /// <summary>Forces this <see cref="BasePlayer" /> to go back to class selection.</summary>
    ...
    /// <summary>Display the cursor and allow this <see cref="BasePlayer" /> to select a text draw.</summary>
    ...
    /// <summary>Cancel text draw selection with the mouse for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>This function plays a crime report for this <see cref="BasePlayer" /> - just like in single-player when CJ commits a crime.</summary>
    ...
    /// <summary>Play an 'audio stream' for this <see cref="BasePlayer" />. Normal audio files also work (e.g. MP3).</summary>
    ...
    /// <summary>Stops the current audio stream for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Loads or unloads an interior script for this <see cref="BasePlayer" />. (for example the ammunation menu)</summary>
    ...
    /// <summary>Set the skill level of a certain weapon type for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Attach an object to a specific bone on this <see cref="BasePlayer" />.</summary>
        ...
        Color materialcolor2)
    {
        ...
        return PlayerInternal.Instance.SetPlayerAttachedObject(Id, index, modelid, (int)bone, offset.X, offset.Y, offset.Z, rotation.X, rotation.Y, rotation.Z,
            scale.X, scale.Y, scale.Z, materialcolor1.ToInteger(ColorFormat.ARGB), materialcolor2.ToInteger(ColorFormat.ARGB));
    }

    /// <summary>Remove an attached object from this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Check if this <see cref="BasePlayer" /> has an object attached in the specified index (slot).</summary>
    ...
    /// <summary>Creates a chat bubble above this <see cref="BasePlayer" />'s name tag.</summary>
    ...
    /// <summary>Puts this <see cref="BasePlayer" /> in a <see cref="BaseVehicle" />.</summary>
    ...
    /// <summary>Puts this <see cref="BasePlayer" /> in a vehicle as driver.</summary>
    /// <param name="vehicle">The vehicle for the player to be put in.</param>
    ...
    /// <summary>Removes/ejects this <see cref="BasePlayer" /> from his vehicle.</summary>
    /// <remarks>
    ...
    /// <summary>Toggles whether this <see cref="BasePlayer" /> can control themselves, basically freezes them.</summary>
    ...
    /// <summary>Plays the specified sound for this <see cref="BasePlayer" /> at a specific point.</summary>
    ...
    /// <summary>Plays the specified sound for this <see cref="BasePlayer" />.</summary>
    /// <param name="soundid">The sound to play.</param>
    ...
    /// <summary>Apply an animation to this <see cref="BasePlayer" />.</summary>
    /// <remarks>
    ...
    public virtual void ApplyAnimation(string animlib, string animname, float fDelta, bool loop, bool lockx, bool locky, bool freeze, int time, bool forcesync)
    {
        AssertNotDisposed();
    ...
    /// <param name="freeze">Will freeze the player in position after the animation finishes.</param>
    /// <param name="time">Timer in milliseconds. For a never ending loop it should be 0.</param>
    public virtual void ApplyAnimation(string animlib, string animname, float fDelta, bool loop, bool lockx, bool locky, bool freeze, int time)
    {
        AssertNotDisposed();
    ...
    /// <summary>Clears all animations for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Get the animation library/name this <see cref="BasePlayer" /> is playing.</summary>
    ...
    /// <summary>Sets a checkpoint (red circle) for this <see cref="BasePlayer" />. Also shows a red blip on the radar.</summary>
    /// <remarks>
    ...
    /// <summary>Disable any initialized checkpoints for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Creates a race checkpoint. When this <see cref="BasePlayer" /> enters it, the <see cref="EnterRaceCheckpoint" /> callback is called.</summary>
    ...
    public virtual void SetRaceCheckpoint(CheckpointType type, Vector3 point, Vector3 nextPosition, float size)
    {
        ...
        PlayerInternal.Instance.SetPlayerRaceCheckpoint(Id, (int)type, point.X, point.Y, point.Z, nextPosition.X, nextPosition.Y, nextPosition.Z, size);
    }

    /// <summary>Disable any initialized race checkpoints for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Set the world boundaries for this <see cref="BasePlayer" /> - players can not go out of the boundaries.</summary>
    ...
    /// <summary>Change the color of this <see cref="BasePlayer" />'s name tag and radar blip for another Player.</summary>
    ...
    /// function like this on a global level, <see cref="BaseMode.ShowNameTags" /> function.
    /// </summary>
    /// <remarks><see cref="BaseMode.ShowNameTags" /> must be set to <c>true</c> to be able to show name tags with <see cref="ShowNameTagForPlayer" />.</remarks>
    ...
    /// <summary>Set the direction this <see cref="BasePlayer" />'s camera looks at. To be used in combination with <see cref="CameraPosition" />.</summary>
    ...
    /// <summary>Move this <see cref="BasePlayer" />'s camera from one position to another, within the set time.</summary>
    ...
    /// <summary>Interpolate this <see cref="BasePlayer" />'s camera's 'look at' point between two coordinates with a set speed.</summary>
    /// <param name="from">The position the camera should start to move from.</param>
    /// <param name="to">The position the camera should move to.</param>
    ...
    /// <summary>Checks if this <see cref="BasePlayer" /> is in a specific <see cref="BaseVehicle" />.</summary>
    ...
    /// <summary>Toggle stunt bonuses for this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Toggle this <see cref="BasePlayer" />'s spectate mode.</summary>
    ...
    /// <summary>Makes this <see cref="BasePlayer" /> spectate (watch) another player.</summary>
    /// <remarks>Order is CRITICAL! Ensure that you use <see cref="ToggleSpectating" /> before <see cref="SpectatePlayer(BasePlayer,SpectateMode)" />.</remarks>
    ...
    /// <summary>Sets this <see cref="BasePlayer" /> to spectate another vehicle, i.e. see what its driver sees.</summary>
    /// <remarks>Order is CRITICAL! Ensure that you use <see cref="ToggleSpectating" /> before <see cref="SpectateVehicle(BaseVehicle,SpectateMode)" />.</remarks>
    ...
    /// <summary>Starts recording this <see cref="BasePlayer" />'s movements to a file, which can then be reproduced by an NPC.</summary>
    /// <param name="recordtype">The type of recording.</param>
    /// <param name="recordname">
    /// Name of the file which will hold the recorded data. It will be saved in "scriptfiles" directory with an automatically added .rec
    /// extension.
    /// </param>
    ...
    /// <summary>Stops all the recordings that had been started with <see cref="StartRecordingPlayerData" /> for this <see cref="BasePlayer" />.</summary>
    ...
    {
        PlayerInternal.Instance.SetPlayerMapIcon(Id, iconId, position.X, position.Y, position.Z, (int)markerType, color, (int)markerStyle);
    }

    /// <summary>Removes a map icon that was set earlier for this player using <see cref="SetMapIcon" />.</summary>
    /// <param name="iconId">The icon identifier.</param>
    ...
    /// <summary>
    /// This function sends a message to this <see cref="BasePlayer" /> with a chosen color in the chat. The whole line in the chat box will be in the set
    ...
    /// <summary>
    /// This function sends a message to this <see cref="BasePlayer" /> with a chosen color in the chat. The whole line in the chatbox will be in the set
    /// color unless color embedding is used.
    /// </summary>
    /// <param name="color">The color of the message.</param>
    ...
    /// <summary>
    /// This function sends a message to this <see cref="BasePlayer" /> in white in the chat. The whole line in the chat box will be in the set color unless
    /// color embedding is used.
    /// </summary>
    /// <param name="message">The text that will be displayed.</param>
    ...
    /// <summary>Displays a message in chat to all players.</summary>
    /// <param name="color">The color of the message.</param>
    ...
    /// <summary>Displays a message in white in chat to all players.</summary>
    ...
    /// <summary>Kicks this <see cref="BasePlayer" /> from the server. They will have to quit the game and re-connect if they wish to continue playing.</summary>
    ...
    /// <summary>
    /// Ban this <see cref="BasePlayer" />. The ban will be IP-based, and be saved in the samp.ban file in the server's root directory.
    /// <see cref="Ban(string)" /> allows you to ban with a reason, while you can ban and unban IPs using the RCON "banip" and "unbanip" commands.
    /// </summary>
    ...
    /// <summary>Ban this <see cref="BasePlayer" /> with a reason.</summary>
    ...
    /// <summary>
    /// Sends a message in the name this <see cref="BasePlayer" /> to another player on the server. The message will appear in the chat box but can only be
    /// seen by <paramref name="receiver" />. The line will start with the this Player's name in his color, followed by the <paramref name="message" /> in white.
    /// </summary>
    ...
    /// <summary>
    /// Sends a message in the name of this <see cref="BasePlayer" /> to all other players on the server. The line will start with the this
    ...
    /// <summary>Shows 'game text' (on-screen text) for a certain length of time for this <see cref="BasePlayer" />.</summary>
    /// <param name="text">The text to be displayed.</param>
    /// <param name="time">The duration of the text being shown in milliseconds.</param>
    /// <param name="style">The style of text to be displayed.</param>
    ...
    public static void CreateExplosionForAll(Vector3 position, ExplosionType type, float radius, int interior, int virtualworld)
    {
    ...
    /// <summary>
    /// Creates an explosion for a <see cref="BasePlayer" />. Only the specific player will see explosion and feel its effects. This is useful when you want
    /// to isolate explosions from other players or to make them only appear in specific virtual worlds.
    /// </summary>
    /// <param name="position">The position of the explosion.</param>
    ...
    /// <summary>Adds a death to the kill feed on the right-hand side of the screen of this <see cref="BasePlayer" />.</summary>
    ...
    /// <summary>Adds a death to the kill feed on the right-hand side of the screen.</summary>
    ...
    /// <summary>Raises the <see cref="Cleanup" /> event.</summary>
    /// <param name="e">An <see cref="DisconnectEventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="Text" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="CommandText" /> event.</summary>
    /// <param name="e">An <see cref="TextEventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="RequestClass" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="EnterVehicle" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="ExitVehicle" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="EnterCheckpoint" /> event.</summary>
    /// <param name="e">An <see cref="EventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="LeaveCheckpoint" /> event.</summary>
    /// <param name="e">An <see cref="EventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="EnterRaceCheckpoint" /> event.</summary>
    /// <param name="e">An <see cref="EventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="LeaveRaceCheckpoint" /> event.</summary>
    /// <param name="e">An <see cref="EventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="RequestSpawn" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="EnterExitModShop" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="SelectedMenuRow" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="ExitedMenu" /> event.</summary>
    /// <param name="e">An <see cref="EventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="InteriorChanged" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="KeyStateChanged" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="Update" /> event.</summary>
    /// <param name="e">An <see cref="PlayerEventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="StreamIn" /> event.</summary>
    /// <param name="e">An <see cref="PlayerEventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="StreamOut" /> event.</summary>
    /// <param name="e">An <see cref="PlayerEventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="DialogResponse" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="ClickMap" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="ClickTextDraw" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="ClickPlayerTextDraw" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="CancelClickTextDraw" /> event.</summary>
    /// <param name="e">An <see cref="PlayerEventArgs" /> that contains the event data. </param>
    ...
    /// <summary>Raises the <see cref="ClickPlayer" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="EditGlobalObject" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="EditPlayerObject" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="EditAttachedObject" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="SelectGlobalObject" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="SelectPlayerObject" /> event.</summary>
    ...
    /// <summary>Raises the <see cref="PickUpPickup" /> event.</summary>
